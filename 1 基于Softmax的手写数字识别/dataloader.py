...
class Dataset(object):
    ...
    def __init__(self, data_root, mode='train', num_classes=10):
        ...
        if mode == 'train':
            # 训练数据集图像和标签
            self.images = images[:55000]  # 形状: (55000, 784)
            self.labels = labels[:55000]  # 形状: (55000,)

        elif mode == 'val':
            # 验证数据集图像和标签
            self.images = images[55000:]  # 形状: (5000, 784)
            self.labels = labels[55000:]  # 形状: (5000, )

        else:
            # 测试数据集
            self.images = images  # 形状: (10000, 784)
            self.labels = labels  # 形状: (5000, )
